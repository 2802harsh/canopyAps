...
let tenantRegisterPost = async (req, res) => {
    ...
    if(!(name && contact && address && password)) return res.redirect("/tenant/register");
    ...
    con.query(query, function (err, result) {
        if (err) {
            console.log(err);
            return res.redirect("/tenant/register");
        }
        console.log(result);
...
let tenantLoginPost = async (req, res) => {
    let contact = req.body.contact;
    let password = req.body.password;
    if(!(contact && password)) return res.redirect("/tenant/login");
